...
function initCommands() {
	let commands = [];
	const commandFiles = fs.readdirSync('./packages/dm_domain/commands', { withFileTypes: true }).filter((file) => file.name.endsWith('.js'));
...
function initEvents() {
	let gameEvents = [];
	const gameEventFiles = fs.readdirSync('./packages/dm_domain/events', { withFileTypes: true }).filter((file) => file.name.endsWith('.js'));
